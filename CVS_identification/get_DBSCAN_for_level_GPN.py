...
def extract_date_from_filename(filename):
    ...
    Ожидаемый формат: sigma_0_R2D_GPN_level_12_YYYY-MM-DD.nc
    """
    patterns = [
        r'sigma_\d+_R2D_'+data_type+r'_level_\d+_(\d{4})-(\d{2})-(\d{2})\.nc',
    ]
...
for sigma in sigma_progress:
    ...
    for month in monthly_progress:
        ...
        ls = list(sorted(Path(rortex_path).glob(f'sigma_{sigma}_R2D_{data_type}_level_{level}_{year}-{month:02d}-*.nc')))
        ...
        for ii, ifile in file_progress:
            try:
                ...
                ds = ds.expand_dims('bottom_top', axis=1)  # Добавляем измерение уровня
