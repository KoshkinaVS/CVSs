...
def find_matching_tracks(polina_tracks, oboltus_tracks, max_distance_km=500, grid_step_km=77):
    """
    Находит совпадающие треки по пересекающимся временным интервалам
    """
    matches = []
    ...
    for polina_id, polina_track in enumerate(tqdm(polina_tracks, desc="Сопоставление треков (Polina -> Oboltus)")):
        ...
        for oboltus_id, oboltus_track in enumerate(oboltus_tracks):
            ...
            # Берем минимальную длину, если по какой-то причине они разные
            ...
            # Выбираем трек с минимальным расстоянием
            if avg_distance < best_match_distance:
                best_match_distance = avg_distance
                best_match = oboltus_id
                best_match_common = min_len
        
        if best_match is not None:
            matches.append({
                'polina_track_id': polina_id,
                'oboltus_track_id': best_match,
                'avg_distance_km': best_match_distance,
                'common_points': best_match_common,
                'polina_start': polina_start,
                'polina_end': polina_end,
                'oboltus_start': oboltus_tracks[best_match]['datetime'].min(),
                'oboltus_end': oboltus_tracks[best_match]['datetime'].max(),
                'polina_points': len(polina_track),
                'oboltus_points': len(oboltus_tracks[best_match])
            })
...
def plot_matched_tracks(oboltus_tracks, polina_tracks, matches, output_dir='matching_plots', ds=None):
    ...
    Теперь Polina - датасет 1, Oboltus - датасет 2
    """
    ...
    for i, match in enumerate(matches):
        try:
            ...
            polina_track = polina_tracks[match['polina_track_id']]      # Polina - датасет 1
            oboltus_track = oboltus_tracks[match['oboltus_track_id']]   # Oboltus - датасет 2
            
            # Рисуем Polina трек (датасет 1) - черным цветом
            ax.plot(polina_track['x'], polina_track['y'], 
                   c='k', linewidth=1.5, label='Polina Track', zorder=5)
            
            # Рисуем Oboltus трек (датасет 2)
            ...
            ax.plot(oboltus_track['x'], oboltus_track['y'], 
                   label=f'Oboltus Track ({int(dist)} km, {time_diff_str})', 
            ...
            if len(oboltus_track) > 0:
                ax.scatter(oboltus_track['x'].values[0], oboltus_track['y'].values[0], marker='*',
                          c='lightgreen', s=7, zorder=10, label='Oboltus start')
                ax.scatter(oboltus_track['x'].values[-1], oboltus_track['y'].values[-1], marker='*',
                          c='orange', s=7, zorder=10, label='Oboltus stop')
            
            # Настраиваем заголовок
            if 'datetime' in polina_track.columns:
                track_start = pd.to_datetime(polina_track['datetime']).min()
                start_str = track_start.strftime('%Y-%m-%dT%H')
                ax.set_title(f'Polina Track {match["polina_track_id"]+1} at {start_str}')
            else:
                ax.set_title(f'Polina Track {match["polina_track_id"]+1}')
            ...
            # Сохраняем
            track_num = match['polina_track_id'] + 1
            filename = f'{output_dir}/Polina_{track_num:09d}_matched.png'
...
def plot_worst_matches_detailed(oboltus_tracks, polina_tracks, matches, output_dir='detailed_plots', 
    ...
    # Сортируем по расстоянию (от худших к лучшим - т.е. от больших расстояний к меньшим)
...
def create_matching_statistics(polina_tracks, oboltus_tracks, matches):
    ...
    stats = {
        ...
        'oboltus_coverage': len(set([m['oboltus_track_id'] for m in matches])) / len(oboltus_tracks) * 100 if oboltus_tracks else 0
    }
    ...
    if matches:
        ...
        # Убираем блок с match_score, так как его больше нет
        
        # Анализ начала и конца треков
        start_diffs = []
        end_diffs = []
        for m in matches:
            start_diff = abs((m['polina_start'] - m['oboltus_start']).total_seconds() / 3600)
            end_diff = abs((m['polina_end'] - m['oboltus_end']).total_seconds() / 3600)
...
def save_matching_results(polina_tracks, oboltus_tracks, matches, stats, output_dir='matching_results'):
    ...
    # Сохраняем детальную информацию по каждому треку Polina
    polina_matching_info = []
    for polina_id, polina_track in enumerate(polina_tracks):
        matching_track = next((m for m in matches if m['polina_track_id'] == polina_id), None)
        ...
        info = {
            ...
            'polina_end': polina_track['datetime'].max(),
            'polina_points': len(polina_track),
            'polina_center_x': polina_track['x'].mean(),
            'polina_center_y': polina_track['y'].mean(),
            'has_match': matching_track is not None
        }
        
        if matching_track:
            info.update({
                'oboltus_track_id': matching_track['oboltus_track_id'],
                'avg_distance_km': matching_track['avg_distance_km'],
                'common_points': matching_track['common_points'],
                'oboltus_start': matching_track['oboltus_start'],
                'oboltus_end': matching_track['oboltus_end']
            })
        
        polina_matching_info.append(info)
    
    polina_df = pd.DataFrame(polina_matching_info)
    polina_df.to_csv(os.path.join(output_dir, 'polina_tracks_matching_info.csv'), index=False)
...
def main():
    ...
    matches = find_matching_tracks(polina_tracks, oboltus_tracks, 
                                  max_distance_km=400, grid_step_km=77) 
    ...
    output_dir = f'{path_init}/data/TC_tracks/Polina_Oboltus_matching_results'
