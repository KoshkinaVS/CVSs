...
output_dir = f"{path_data_dir}/{data_type}/EddyClicker_clustering_r2d/{scaler_type}_scaler/EddyClicker_{clustering_type}_results_2010_2026-05-13/{params_type}"
...
if data_type == 'LoRes':
    ...
    ncfile = f'{path_dir_raw}/wrfout_d01_2019-02-01_00:00:00'
...
# Автоматическое создание списка с суффиксами
param_cols = []
for param in base_params:
    param_cols.append(f'{param}_start')
    param_cols.append(f'{param}_max')
    param_cols.append(f'{param}_stop')

param_cols.append('vel_at_max_kmh')
param_cols.append('track_len_total')
...
def process_season(season_name, months, n_clusters_range, clustering_type='gmm', scaler_type='standart'):
    ...
    for n_clusters in tqdm(n_clusters_range, desc=f"{season_name} ({clustering_type})"):
        ...
        mask_bad = df_season['vertical_shear_strength_max'].isna()
        n_bad = mask_bad.sum()
