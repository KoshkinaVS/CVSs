"""
2026-09-07 (три правки в порядке от старой к новой):
1. Легенда Cyclones/Anticyclones заменена на крупную русскую "Ц: NN" /
   "АЦ: MM" (см. plot_R2D: блок legend_elements/ax.legend), по аналогии с
   тем же изменением в отдельном ERA5-скрипте
   plot_R2D_maps_no_rads_2026-09-07.py. Окружности радиусов вихрей в этом
   скрипте и так не рисуются (см. "История файла" ниже, п.4) — переносить
   тут нечего, "_no_rads"-варианта не создавалось.
2. Заливка R2D замаскирована по ИТОГОВОМУ полю cluster (после size_filter,
   см. filter_clusters_by_size): точки, не попавшие ни в один кластер,
   прошедший size_filter (шум DBSCAN либо отфильтрованный по размеру
   кластер), в заливке R2D больше не показываются — раньше маскировалось
   только field_2d == 0, из-за чего "отфильтрованные" вихри всё ещё были
   видны цветом, просто без контура/звезды (см. plot_R2D, cluster_mask
   перед field_masked). Для LoRes/HiRes cluster_mask и field_2d — из разных
   .nc, при несовпадении формы (не должно происходить, но не проверено на
   реальных данных) скрипт откатывается на старую маску field_2d==0 и
   печатает предупреждение, а не падает.
3. Легенда Ц/АЦ теперь поддерживает ДВА языка — русский (крупный, как в
   п.1) и английский (Cyclones/Anticyclones, шрифт увеличен НЕМНОГО, по
   просьбе пользователя "чуть увеличь текст", в отличие от русской версии,
   которая была сделана крупной по отдельной, более ранней просьбе) — см.
   LEGEND_VARIANTS рядом с ENVELOPE_VARIANTS. Выбирается через
   --legend-lang (en / ru / both, по умолчанию both — рисует ОБА варианта
   на каждую комбинацию, как envelope-варианты). Имя файла получает
   суффикс языка сразу после env-{variant}: "_en" или "_rus" (см.
   render_era5/render_wrf).

Остальная логика (envelope-варианты none/hull/exact, LoRes/HiRes/ERA5,
extent и т.д.) не менялась — см. plot_R2D_maps_20260905.py как исходную,
до-модификационную версию скрипта.
...
Каждая карта — один PNG на комбинацию (envelope-вариант x язык легенды,
2026-09-07, см. LEGEND_VARIANTS/--legend-lang): суффикс env-{variant}
(hull/exact/none, см. Раунд 6) + суффикс языка легенды (_en/_rus):
  ERA5:          R2D_map_ERA5_eps{eps}_sf{size_filter}_env-{variant}_{lang}_{date}_{hour}00.png
  LoRes / HiRes: R2D_map_{data_type}_sigma_{sigma}_sf{size_filter}_env-{variant}_{lang}_{date}_{hour}00.png
  (например: ..._env-hull_en_2010-08-28_1200.png / ..._env-hull_rus_2010-08-28_1200.png)
"""
...
# Язык легенды Ц/АЦ, генерируется АВТОМАТИЧЕСКИ на каждом запуске (как и
# ENVELOPE_VARIANTS) — добавлено 2026-09-07 по прямой просьбе пользователя
# ("пусть будет флаг выбора языка и сохраняй с именами _en _rus"). "ru" —
# крупная русская легенда ("Ц: NN АЦ: MM"), по более ранней просьбе того же
# дня; "en" — оригинальная английская (Cyclones/Anticyclones), шрифт
# увеличен НЕМНОГО ("чуть увеличь"), в отличие от "ru". file_suffix уходит
# в имя файла сразу после суффикса env-{variant} (см. render_era5/
# render_wrf) — порядок ключей ниже задаёт порядок --legend-lang both по
# умолчанию (сначала en, потом ru, для каждого envelope-варианта).
LEGEND_VARIANTS: dict[str, dict] = {
    "en": {
        "cyclone_label": "Cyclones: {n}",
        "anticyclone_label": "Anticyclones: {n}",
        "fontsize": 13,
        "handlelength": 1.8,
        "handleheight": 1.2,
        "borderpad": 0.6,
...
def plot_R2D(
    ...
    legend_lang: "ru" | "en" — язык и оформление легенды Ц/АЦ, см.
    LEGEND_VARIANTS (2026-09-07).
    """
    ax.set_global()
    ...
    # Маска R2D по ИТОГОВОМУ полю cluster (после size_filter, см.
    # filter_clusters_by_size, 2026-09-07): точки, не попавшие ни в один
    # кластер, прошедший size_filter (cluster_mask == 0 — DBSCAN-шум либо
    # кластер, отфильтрованный как слишком мелкий), в заливке R2D вообще не
    # показываются — раньше маскировалось только field_2d == 0, из-за чего
    # "отфильтрованные" вихри всё равно оставались видны цветом, просто без
    # контура/звезды. Для LoRes/HiRes cluster_mask и field_2d приходят из
    # РАЗНЫХ .nc (см. докстринг модуля/render_wrf) — если формы почему-то не
    # совпадают, отступаем к старой маске (field_2d == 0), чтобы не уронить
    # рендер, и печатаем предупреждение.
    if cluster_mask.shape == field_2d.shape:
        field_masked = np.where(cluster_mask == 0, np.nan, field_2d)
    else:
        print(f"  ВНИМАНИЕ: форма cluster {cluster_mask.shape} не совпадает с полем R2D "
              f"{field_2d.shape} для '{name}' — маска по cluster не применена, "
              f"использована старая маска (R2D == 0).")
    ...
    if n_clusters > 0:
        ...
        # Легенда Ц/АЦ на выбранном языке (en/ru, см. LEGEND_VARIANTS,
        # 2026-09-07) — русская версия крупная (по более ранней просьбе),
        # английская увеличена лишь слегка ("чуть увеличь текст").
        lv = LEGEND_VARIANTS[legend_lang]
        legend_elements = [
            Patch(facecolor="limegreen", alpha=0.7, label=lv["cyclone_label"].format(n=n_clusters_pos)),
            Patch(facecolor="deeppink", alpha=0.7, label=lv["anticyclone_label"].format(n=n_clusters_neg)),
        ]
        legend = ax.legend(handles=legend_elements, loc="upper left", fontsize=lv["fontsize"], framealpha=0.9,
                            handlelength=lv["handlelength"], handleheight=lv["handleheight"],
                            borderpad=lv["borderpad"], labelspacing=lv["labelspacing"])
...
def render_era5(eps: int, size_filter: int, date: str, hour: int, out_dir: Path,
                 ...
                 max_contour_points: int, legend_langs: list[str]) -> list[Path]:
    """legend_langs: список языков легенды (см. LEGEND_VARIANTS) — на каждый
    рисуется ОТДЕЛЬНЫЙ PNG (2026-09-07), но .nc читается и общие для всех
    языков массивы (field_2d/lon2d/lat2d/vmax) считаются ОДИН раз."""
    ...
    try:
        ...
        for lang in legend_langs:
            ...
            plot_R2D(
                ...
                max_contour_points=max_contour_points, legend_lang=lang,
            )

            lang_suffix = LEGEND_VARIANTS[lang]["file_suffix"]
            out_path = out_dir / (f"R2D_map_ERA5_eps{eps:02d}_sf{size_filter:02d}_env-{env_tag}"
                                   f"{lang_suffix}_{date}_{hour:02d}00.png")
            fig.savefig(out_path, dpi=200, bbox_inches="tight")
            plt.close(fig)
            print(f"[ERA5 eps={eps} size_filter={size_filter} env={env_tag} legend={lang}] сохранено: {out_path}")
...
def render_wrf(data_type: str, date: str, hour: int, sigma: int, size_filter: int, out_dir: Path,
                ...
                legend_langs: list[str],
                samples_per_day: int | None = None) -> list[Path]:
    ...
    legend_langs: список языков легенды (см. LEGEND_VARIANTS) — на каждый
    рисуется ОТДЕЛЬНЫЙ PNG (2026-09-07), но оба .nc читаются и общие для
    всех языков массивы считаются ОДИН раз."""
    ...
    try:
        ...
        for lang in legend_langs:
            ...
            lang_suffix = LEGEND_VARIANTS[lang]["file_suffix"]
            out_path = out_dir / (f"R2D_map_{data_type}_sigma_{sigma}_sf{size_filter:02d}_env-{env_tag}"
                                   f"{lang_suffix}_{date}_{hour:02d}00.png")
            fig.savefig(out_path, dpi=200, bbox_inches="tight")
            plt.close(fig)
            print(f"[{data_type} size_filter={size_filter} env={env_tag} legend={lang}] сохранено: {out_path}")
            out_paths.append(out_path)
        return out_paths
    finally:
        cluster_ds.close()
...
def main() -> None:
    ...
    parser.add_argument("--vmax-frac", type=float, default=0.2,
                         help="доля от max|R2D| для границ цветовой шкалы — чем меньше, тем "
                              ...
                              "были ярче).")
    parser.add_argument("--legend-lang", nargs="+", choices=list(LEGEND_VARIANTS.keys()) + ["both"],
                         default=["both"],
                         help="язык легенды Ц/АЦ — en (Cyclones/Anticyclones, шрифт увеличен "
                              "немного), ru (крупная 'Ц: NN АЦ: MM') или both (оба сразу, "
                              "по умолчанию, 2026-09-07): для каждой комбинации "
                              "(data_type/eps, size_filter, envelope-вариант) сохраняется по "
                              "одной картинке на язык, с суффиксом _en/_rus в имени файла "
                              "сразу после env-{variant}.")
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    args = parser.parse_args()

    # "both" разворачиваем в список языков ОДИН раз здесь — дальше по коду
    # (render_era5/render_wrf) везде используется уже готовый список.
    if "both" in args.legend_lang:
        legend_langs = list(LEGEND_VARIANTS.keys())
    else:
        legend_langs = list(dict.fromkeys(args.legend_lang))  # без дублей, порядок сохранён
    ...
    print(f"Языки легенды Ц/АЦ: {legend_langs}")
        ...
        if "ERA5" in args.data_types:
            for eps in args.eps:
                for size_filter in args.size_filter:
                    paths = render_era5(eps, size_filter, args.date, args.hour, args.out_dir,
                                         draw_envelope, envelope_method, args.vmax_frac,
                                         args.max_contour_points, legend_langs)
                    saved.extend(paths)

        if "LoRes" in args.data_types:
            for size_filter in args.size_filter:
                paths = render_wrf("LoRes", args.date, args.hour, args.wrf_sigma, size_filter, args.out_dir,
                                    draw_envelope, envelope_method, args.vmax_frac,
                                    args.max_contour_points,
                                    args.lores_cluster_nc, args.lores_r2d_nc, legend_langs,
                                    samples_per_day=args.wrf_samples_per_day)
                saved.extend(paths)

        if "HiRes" in args.data_types:
            for size_filter in args.size_filter:
                paths = render_wrf("HiRes", args.date, args.hour, args.wrf_sigma, size_filter, args.out_dir,
                                    draw_envelope, envelope_method, args.vmax_frac,
                                    args.max_contour_points,
                                    args.hires_cluster_nc, args.hires_r2d_nc, legend_langs,
                                    samples_per_day=args.wrf_samples_per_day)
                saved.extend(paths)
