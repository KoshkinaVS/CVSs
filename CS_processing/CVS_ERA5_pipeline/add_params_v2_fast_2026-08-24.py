...
3. ПАРАЛЛЕЛИЗМ ПО СУТКАМ.
   Внутри одного батча треков (см. п.4) точки группируются по календарной
   дате (это естественная граница GRIB-файла — один файл = одни сутки) и
   раздаются по процессам через persistent `multiprocessing.Pool`.

4. БАТЧИНГ ПО ДАТЕ СТАРТА ТРЕКА (память).
   Раньше все ~26000 CSV с треками читались в память СРАЗУ, и результаты по
   ВСЕМ точкам ВСЕХ треков копились в одном списке, прежде чем что-либо
   сохранялось на диск. Теперь файлы треков сортируются по дате старта
   (парсится из имени `NNNNNN_track_YYYY-MM-DDTHH.csv`) и режутся на батчи по
   `--tracks-per-batch` штук (по умолчанию 500). В памяти одновременно
   держится только один батч: его DataFrame-ы, его точки, его результаты.
   После обработки батча CSV сразу пишутся на диск и батч выгружается.
   Пул процессов при этом создаётся ОДИН раз на весь запуск (не на каждый
   батч), а кэш открытых ERA5-файлов внутри каждого воркера больше не
   сбрасывается после каждых суток — соседние батчи (отсортированные по дате)
   с высокой вероятностью продолжают попадать в те же месячные NetCDF-файлы.
...
class CycloneProcessorERA5:
    ...
    def _process_point(
        ...
        #
        # ВАЖНО: tropopause (dyn_z) лежит на СВОЕЙ сетке (0.3°, 601x1201,
        # -180..180), отличной от общей сетки mslp/uv10m/t2/precip/blh/GRIB
        # (0.25°, 721x1440, 0..360), под которую построен общий `aggregator`.
        # Использовать для tropopause общий aggregator нельзя — позиционные
        # индексы (lat_idx/lon_idx), посчитанные для 721x1440 сетки, при
        # применении к массиву 601x1201 либо указывают не в ту физическую
        # точку (если индекс попал в границы), либо кидают IndexError (тогда
        # try/except в process_day_points тихо обнуляет ВСЕ параметры точки,
        # не только pbl_trop_frac). Поэтому под tropopause строится отдельный
        # aggregator на его собственной сетке. (fix 2026-09-02)
        # ------------------------------------------------------------------
        if blh is not None and tropopause is not None:
            ...
            trop_aggregator = SpatialAggregatorERA5(
                center_lat=center_lat,
                center_lon=center_lon,
                radius_deg=radius_deg,
                grid_lat=tropopause[trop_lat_name].values,
                grid_lon=tropopause[trop_lon_name].values,
            )
            blh_val = aggregator.aggregate(blh, AggregationMethod.MEDIAN)
            trop_val = trop_aggregator.aggregate(tropopause, AggregationMethod.MEDIAN)
...
# ============================================================================
# Батчинг треков по дате старта (чтобы не держать все 25000+ файлов в памяти)
# ============================================================================

# Имя вида: 025906_track_2010-12-31T05.csv
_TRACK_START_RE = re.compile(r"_track_(\d{4}-\d{2}-\d{2})")


def _parse_track_start_date(track_file: str) -> Optional[date]:
    m = _TRACK_START_RE.search(Path(track_file).name)
    if not m:
        return None
    try:
...
def _chunk_track_files(track_files, tracks_per_batch: int) -> List[list]:
    """
    Сортирует файлы треков по дате старта (из имени файла) и режет на батчи
    фиксированного размера. Сортировка по дате даёт побочный эффект: соседние
    батчи, скорее всего, попадают в один и тот же месяц/соседние дни ERA5,
    поэтому кэш открытых Dataset между батчами продолжает "попадать" в те же
    файлы, а не скачет туда-сюда.

    Файлы без распознанной даты в имени уходят в конец списка (все вместе).
    """
    with_date = []
    without_date = []
    for f in track_files:
        d = _parse_track_start_date(f)
        if d is None:
            without_date.append(f)
        else:
            with_date.append((d, f))

    with_date.sort(key=lambda x: x[0])
    ordered = [f for _, f in with_date] + without_date

    return [
        ordered[i : i + tracks_per_batch]
        for i in range(0, len(ordered), tracks_per_batch)
    ]


def _read_track_batch(batch_files: list):
    """
    Читает CSV только для файлов ОДНОГО батча.

    track_id здесь локальный для батча (0..len(batch_files)-1) — это ок, т.к.
    результаты записываются и файл сохраняется сразу после обработки батча,
    id не должен быть глобально уникальным между батчами.
    """
    tracks = {}
    points = []

    for track_id, track_file in enumerate(batch_files):
        try:
            ...
            tracks[track_id] = {"file": track_file, "df": df}
...
def process_all_tracks(
    ...
    n_workers: int = 4,
    max_open_datasets: int = 8,
    tracks_per_batch: int = 500,
):
    """
    Обрабатывает треки БАТЧАМИ по ~tracks_per_batch штук, отсортированными по
    дате старта трека (из имени файла). В памяти одновременно держится только
    один батч: его DataFrame-ы, его точки и его результаты. После обработки
    батча CSV сразу сохраняются на диск и батч выгружается из памяти.

    Пул процессов создаётся ОДИН раз на весь запуск (а не на каждый батч),
    чтобы воркеры и их внутренний LRU-кэш открытых ERA5-файлов переиспользовались
    между батчами — соседние батчи (по дате старта) с высокой вероятностью
    трогают те же месячные/дневные файлы ERA5.
    """
    output_dir.mkdir(parents=True, exist_ok=True)

    batches = _chunk_track_files(track_files, tracks_per_batch)
    logger.info(
        "Всего треков: %d; батчей: %d (~%d треков в батче); воркеров: %d",
        len(track_files),
        len(batches),
        tracks_per_batch,
        n_workers,
    ...
    try:
        for batch_files in tqdm(batches, desc="Батчи треков"):
            tracks, points = _read_track_batch(batch_files)

            if not points:
                continue

            # Группировка точек ЭТОГО батча по календарным суткам.
            day_buckets: Dict[Tuple[int, int, int], list] = defaultdict(list)
            for item in points:
                t = item[0]
                day_buckets[(t.year, t.month, t.day)].append(item)
            day_batches = list(day_buckets.values())

            batch_results: List[Tuple[int, int, Dict[str, Any]]] = []
            if pool is not None:
                for r in pool.imap_unordered(_worker_process_day, day_batches):
                    batch_results.extend(r)
            else:
                for db in day_batches:
                    batch_results.extend(sequential_processor.process_day_points(db))

            # Записываем результаты в DataFrame-ы ТОЛЬКО этого батча.
            for track_id, row_idx, results in batch_results:
                df = tracks[track_id]["df"]
                for param, value in results.items():
                    df.at[row_idx, param] = value

            # Сразу сохраняем и освобождаем память батча.
            ...
            del tracks, points, day_buckets, day_batches, batch_results
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Сколько треков читать в память и обрабатывать за один батч.",
    )
    ...
    if not track_files:
        ...
        print(f"Воркеров: {args.workers}, треков в батче: {args.tracks_per_batch}")
