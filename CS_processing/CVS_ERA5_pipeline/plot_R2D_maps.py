...
с радиусами вихрей — портировано из plot_R2D() в ERA5_plot_test.ipynb (положен
пользователем в папку TempestExtremes/) для визуального сравнения всей сетки
eps x size_filter (6_grid_runner_TE_ERA5.py: EPS_VALUES x SIZE_FILTER_VALUES).

Задача
------
Один и тот же временной срез (по умолчанию 2010-08-28 12:00) — 12 отдельных
PNG: по 2 на комбинацию (eps=1/2) x (size_filter=10/25/49) — полная версия
(с окружностями радиусов вихрей и подписями км) и версия "_no_rads" (только
контуры кластеров ConvexHull, без окружностей/подписей радиусов) — см.
RENDER_VARIANTS.
...
Пример запуска
--------------
python plot_R2D_maps.py                              # вся сетка (2x3x2=12 PNG), 2010-08-28 12:00
python plot_R2D_maps.py --date 2010-08-28 --hour 12
python plot_R2D_maps.py --eps 1 --size-filter 10 25   # сузить сетку
python plot_R2D_maps.py --out-dir /storage/.../my_maps
python plot_R2D_maps.py --center-name center          # звёздочки = глобальный центр кластера
                                                        # (по умолчанию — local_extr_cluster,
                                                        # как в последнем состоянии ноутбука)

Каждая карта сохраняется в двух вариантах (см. RENDER_VARIANTS):
  R2D_eps{eps}_sf{size_filter}_{date}_{hour}00.png           — полная (радиусы+подписи)
  R2D_eps{eps}_sf{size_filter}_{date}_{hour}00_no_rads.png   — только контуры кластеров
"""
...
def plot_R2D(ax, cluster_ds_t: xr.Dataset, vmax: float, center_name: str, name: str,
    ...
    # filter_clusters_by_size): точки, которые не попали ни в один кластер,
    # прошедший фильтр по size_filter (cluster_mask == 0 — это либо
    # DBSCAN-шум, либо кластер, отфильтрованный как слишком мелкий), в
    # заливке R2D вообще не показываются — раньше маскировалось только
    # R2D == 0, из-за чего "отфильтрованные" вихри всё равно оставались
    # видны цветом на карте, просто без контура/звезды.
    clusters = np.where(cluster_mask == 0, np.nan, cluster_ds_t["R2D"].values)
...
# (show_radius, suffix для имени файла) — по умолчанию рендерим обе версии
# каждой карты: полную (окружности радиусов + подписи км) и "no_rads"
# (только контуры кластеров ConvexHull — draw_contours у plot_R2D всегда True,
# убирается только show_radius).
RENDER_VARIANTS = [
    (True, ""),
    (False, "_no_rads"),
]
