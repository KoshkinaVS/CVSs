...
def plot_track_with_DBSCAN(ds, ds_speed, dt_step, crit_vals, local_extr, 
                         ...
                         name_pics, show_radius=False, save=True):
    ...
    for t in tqdm(range(start_time, stop_time, dt_step_int), 
        ...
        #### 2026-08-19
        plot_crit(ax, ds, crit_vals, local_extr, t, show_radius=show_radius)
...
#### 2026-08-19 upd for rad,r2d,wspd in files
def load_season_tracks(year, months, path_data):
    """
    Загрузка треков из файлов TempestExtremes в формате .txt с фильтрацией по месяцам
    ...
    for line in lines:
        if line.startswith('start'):
            ...
            if len(parts) >= 10:
                ...
                current_track['lon'].append(float(parts[2]))
                current_track['lat'].append(float(parts[3]))
                current_track['rad'].append(float(parts[4]))
                current_track['r2d'].append(float(parts[5]))
                current_track['wspd'].append(float(parts[6]))
                
                current_track['year'].append(int(parts[7]))
                current_track['month'].append(point_month)
                current_track['day'].append(int(parts[9]))
                current_track['hour'].append(int(parts[10]))
...
def load_season_tracks(year, months, path_data, variables=None):
    """
    Загрузка треков из файлов TempestExtremes.
    ...
    year : int
        Год данных.
    months : list
        Список месяцев для загрузки.
    path_data : str
        Путь к файлу с треками.
    variables : list, optional
        Дополнительные переменные, записанные после lat.
        Например:
            ['wspd', 'r2d']
            ['rad', 'r2d', 'wspd']
            ['rad']
        
        Базовые переменные x, y, lon, lat, year, month, day, hour
        читаются всегда.
    """
    ...
    # Порядок переменных в строке файла:
    # x y lon lat + variables + year month day hour
    variable_start = 4
    ...
    for line in lines:
        ...
        if line.startswith('start'):
            ...
            if current_track is not None and len(current_track['x']) > 0:
            ...
            if len(parts) >= 6:
                ...
                track_month = int(parts[3])
                current_track_months = {track_month}
        ...
        # ---------------------------------------------------------
            ...
            # x y lon lat + variables + year month day hour
            expected_length = 4 + len(variables) + 4
            ...
            try:
                ...
                for i, var in enumerate(variables):
                    current_track[var].append(
                        float(parts[variable_start + i])
                    )

                # Временные данные идут после variables
                time_start = variable_start + len(variables)

                point_year = int(parts[time_start])
                point_month = int(parts[time_start + 1])
                point_day = int(parts[time_start + 2])
                point_hour = int(parts[time_start + 3])

                current_track['year'].append(point_year)
                current_track['month'].append(point_month)
                current_track['day'].append(point_day)
                current_track['hour'].append(point_hour)

                current_track_months.add(point_month)
...
def load_local_extrema_nodes(file_path, variables=None):
    """
    Загрузка локальных экстремумов TempestExtremes Nodes.

    Формат строки:
        x y lon lat [variables...]

    Например:
        x y lon lat r2d
        x y lon lat rad r2d wspd
    """
    ...
    expected_length = 4 + len(variables)
            ...
            if line.startswith('\t'):
                ...
                try:
                    ...
                    if current_time is not None:
                        ...
                        time_slices[current_time].append(point)
...
def plot_crit(ax, ds, crit_vals, local_extr_nodes, t, show_radius=False):
    """
...
#     plt.gca().invert_yaxis()
    ...
    if current_time in local_extr_nodes:
        ...
        lon_points = [point['lon'] for point in local_extr_nodes[current_time]]
        lat_points = [point['lat'] for point in local_extr_nodes[current_time]]
        # r2d_points = [point['r2d'] for point in local_extr_nodes[current_time]]
    ...
    if show_radius:
        ...
        points = local_extr_nodes[current_time]
    
        x_points = [point['lon'] for point in points]
        y_points = [point['lat'] for point in points]
        rad_points = [point['rad'] for point in points]
    
        for x, y, rad in zip(x_points, y_points, rad_points):
    
            if np.isnan(rad) or rad <= 0:
                continue
    
            # Координаты окружности в единицах ячеек сетки
            n_points = 50
            angles = np.linspace(0, 2 * np.pi, n_points)
    
            circle_x = x + 0.25 * rad * np.cos(angles)
            circle_y = y + 0.25 * rad * np.sin(angles)
