...
def update_and_save(time_index, index_t):
    ...
    # ФИКСИРОВАННЫЙ ЭКСТЕНТ для левого графика (20 ячеек сетки ~ 5 градусов * 4)
    # Используем фиксированные координаты, чтобы окно не менялось
    FIXED_LON_MIN = -80
    FIXED_LON_MAX = -30
    FIXED_LAT_MIN = 15
    FIXED_LAT_MAX = 55
    
    # Альтернативный вариант - динамический, но с фиксированным размером в 20 градусов
    # center_lon_fixed = -55  # Центр Атлантики
    # center_lat_fixed = 35
    # FIXED_LON_MIN = center_lon_fixed - 10
    # FIXED_LON_MAX = center_lon_fixed + 10
    # FIXED_LAT_MIN = center_lat_fixed - 10
    # FIXED_LAT_MAX = center_lat_fixed + 10
    ...
    # Рисуем изобары на левом графике
    plot_isobars(ax1, year, month, day, our_time, zoom=False)
    ...
    ax1.plot(current_lon, current_lat, 'ko', markersize=6, 
             transform=ccrs.PlateCarree())
