...
def determine_primary_track_type(
    ...
    """
    Определяет один приоритетный тип для каждого TID по всему
    жизненному циклу трека.

    Классификация основана на точном значении Adjusted_Label:
    ...
    Важно: классификация выполняется ДО пространственно-временного
    фильтра, поэтому full-track и региональные выборки используют
    одну и ту же классификацию TID.
    """
    ...
    # Наличие конкретной стадии где-либо в полном TID
    stage_flags = (
        pd.DataFrame({
            "has_TC": label.eq("TC"),
            "has_SS": label.eq("SS(STLC)"),
            "has_PL": label.eq("PL(PTLC)"),
        }, index=df_all.index)
        .groupby(df_all["TID"])
        .any()
    )
    ...
    # Приоритет TC > SS > PL
    stage_flags["primary_type"] = np.select(
        [
            stage_flags["has_TC"],
            stage_flags["has_SS"],
            stage_flags["has_PL"],
    ...
    stage_flags = stage_flags.reset_index()
...
def select_tracks(
    ...
    """
    Выбирает TID нужного типа среди узлов, попавших в регион/год.

    Тип TID уже определён глобально по всему parquet:
        TC > SS > PL

    Поэтому:
        TC-трек никогда не попадёт в SS/PL;
        SS-трек никогда не попадёт в PL.
    """

    if df_reg.empty:
        log.warning(
            "После пространственно-временного фильтра нет узлов."
        )
        return df_reg, pd.Index([])

    config = TYPE_CONFIG[track_type]
    node_label = config["node_label"]

    # TID, которым глобально присвоен именно этот приоритетный тип
    valid_tid = track_types.loc[
        track_types["primary_type"] == track_type,
        "TID",
    ]

    candidate = df_reg[
        df_reg["TID"].isin(valid_tid)
    ].copy()

    counts = candidate.groupby("TID").size()
    ...
    log.info(
        "Критерий классификации TID: Adjusted_Label == '%s'",
        node_label,
    )
    log.info(
        "Приоритет: TC > SS > PL",
    ...
    log.info(
        "TID с приоритетным типом %s во всем каталоге: %d",
        track_type,
        len(valid_tid),
    )
    log.info(
        "Выбрано TID после регионально-временного фильтра: %d",
        len(selected_tid),
    )
    log.info(
        "Узлов в выбранных TID: %d",
        len(df_selected),
    )

    return df_selected, selected_tid
