...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="NA",
        help="Регион: NA или Arctic. Для NA TC дополнительно фильтруются по genesis.",
    )
...
def determine_primary_track_type(
    ...
    """
    Determine one mutually exclusive type for every full TID.

    Priority:
        TC > SS > PL

    Type is determined from Adjusted_Label over the COMPLETE TID.

    For NA:
        a TC is considered a North Atlantic TC only if the FIRST
        point of the complete TID is an oceanic point in the
        North Atlantic window.

    Thus Pacific-origin systems that later enter the Atlantic are
    excluded from NA TC.
    """

    land_geometry = None
    if region == "NA":
        land_geometry = build_land_geometry()
        if land_geometry is None:
            log.warning(
                "Маска суши недоступна. Для genesis используется "
                "только географическое окно North Atlantic."
            )

    rows = []

    for tid, group in df_all.groupby("TID"):
        ...
        has_tc = labels.eq("TC").any()
        has_ss = labels.eq("SS(STLC)").any()
        has_pl = labels.eq("PL(PTLC)").any()

        # Mutual priority: TC > SS > PL.
        if has_tc:
            primary_type = "TC"
        elif has_ss:
            primary_type = "SS"
        ...
        genesis_time = first["ISOTIME"]
        genesis_lon = float(first["LON"])
        genesis_lat = float(first["LAT"])

        genesis_is_NA = False
        tc_excluded_non_NA_genesis = False

        if primary_type == "TC" and region == "NA":
            genesis_is_NA = is_oceanic_na_genesis(
                genesis_lon,
                genesis_lat,
                land_geometry,
            )

            if not genesis_is_NA:
                # It is a TC, but not a North Atlantic TC.
                # Remove it from the mutually exclusive NA set.
                primary_type = None
                tc_excluded_non_NA_genesis = True
        ...
        rows.append(
            {
                ...
                "genesis_lon_180": normalize_lon_180(genesis_lon),
                "genesis_is_NA": bool(genesis_is_NA),
                "tc_excluded_non_NA_genesis": bool(
                    tc_excluded_non_NA_genesis
                ),
    ...
    if region == "NA":
        log.info(
            "TC, исключённых из NA по genesis: %d",
            int(result["tc_excluded_non_NA_genesis"].sum()),
        )
...
def main() -> int:
    ...
    save_csv(
        selected_genesis,
        args.outdir / f"{stem}_genesis_classification.csv",
    )
