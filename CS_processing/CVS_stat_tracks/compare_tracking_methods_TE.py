...
# Конфигурация для TempestExtremes
tempest_config = {
    'tracking_type': 'TempestExtremes',
    'speed_option': 'default',
    'pref_tracking': 'default',
    'tracks_name': 'Tracks_timefilter_1h'
}
...
# Функция для загрузки треков TempestExtremes
def load_tracks_tempestextremes(path_data_dir):
    """Загружает треки из формата TempestExtremes"""
    # Ищем все файлы треков за 2019 год
    track_files = sorted(glob.glob(f'{path_data_dir}/*tracks_*.txt'))
    
    if not track_files:
        raise FileNotFoundError(f"No track files found in {path_data_dir}")
    ...
    if not all_tracks:
        raise ValueError("No tracks could be loaded")
    
    tracks_all = huracanpy.concat_tracks(all_tracks)
    return tracks_all
...
#                 'total_matches': len(matches),
        # 'n_ref_tracks': int(tracks_EC.track_id.max()),
        # 'n_auto_tracks': int(tracks_year.track_id.max())
#             })
...
print("\n" + "="*70)
print("PROCESSING TEMPESTEXTREMES TRACKS")
print("="*70)

try:
    # Формируем путь для TempestExtremes
    tempest_tracks_name = tempest_config['tracks_name']
    path_tempest = f'/storage/thalassa/users/vkoshkina/data/TempestExtremes/{data_type}/R2D_{data_type}_level_{level}_sigma_{sigma}/{tempest_tracks_name}'
    
    print(f"\nProcessing: {tempest_config['tracking_type']}")
    print(f"Path: {path_tempest}")
    print(f"Level: {level}, Sigma: {sigma}")
    
    # Загружаем треки TempestExtremes
    tracks_year = load_tracks_tempestextremes(path_tempest)
    
    # Фильтруем по региону
    tracks_tempest = huracanpy.trackswhere(
                tracks_year, tracks_year.track_id, lambda x: (x.lon < 100).all()   
            )
    
    print(f"  Loaded {int(tracks_tempest.track_id.max())} TempestExtremes tracks")
    
    # Сравниваем с tracks_EC
    matches = huracanpy.assess.match(
        [tracks_EC, tracks_tempest], 
        names=['manual', 'TempestExtremes'], 
        max_dist=dist_m*5,                 
        min_overlap=3, 
        tracks1_is_ref=True
    )
    ...
    # # Вычисляем среднее расстояние между треками и долю перекрытия
    # mean_distance, overlap_percentage = calculate_track_metrics(
    #     matches, tracks_EC, tracks_tempest,
    #     ref_name='manual',
    #     det_name='TempestExtremes'
    # )
    
    # Сохраняем результаты
    results.append({
        'tracking_type': tempest_config['tracking_type'],
        'speed_option': tempest_config['speed_option'],
        'pref_tracking': f'level_{level}_sigma_{sigma}',
        'POD': pod,
        # 'mean_distance_km': mean_distance,
        # 'overlap_percentage': overlap_percentage,
        'total_matches': len(matches),
        'n_ref_tracks': int(tracks_EC.track_id.max()),
        'n_auto_tracks': int(tracks_tempest.track_id.max())
    })
    ...
    print(f"  Reference tracks: {int(tracks_EC.track_id.max())}")
    print(f"  Tempest tracks: {int(tracks_tempest.track_id.max())}")
    
except Exception as e:
    print(f"  Error processing TempestExtremes: {e}")
    results.append({
        'tracking_type': tempest_config['tracking_type'],
        'speed_option': tempest_config['speed_option'],
        'pref_tracking': f'level_{level}_sigma_{sigma}',
...
output_file = f'{output_dir}/{data_type}_tracking_comparison_results_TE.csv'
