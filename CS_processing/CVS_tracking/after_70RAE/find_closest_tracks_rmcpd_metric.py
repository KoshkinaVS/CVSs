...
def process_configuration(config):
    ...
    folder_name = f'TC_tracks/NAAD_NOAA_tracks_merged_rmcpd_metric/{data_type}_sigma_{sigma}/{tracking_type}_{CVS_speed}_{pref_tracking}'
        ...
        for key in tqdm(TC_dict.keys(), desc=f'Calculating distances {tracking_type} {CVS_speed} {year}'):
            ...
            for idx in range(len(TC_dict[key]['NAAD_tracks'])):
                ...
                if track_NAAD is not None:
                    # Используем rMCPD вместо старой метрики
                    rmcpd_value = calculate_rmcpd(CS_track_NOAA, track_NAAD)
                    dist_list.append(rmcpd_value)
