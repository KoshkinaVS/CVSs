...
def process_single_track(args):
    """Обрабатывает один трек-файл"""
    file_path, params_config_new, data_type, km, output_dir = args
    
    try:
        # Читаем файл и сразу преобразуем datetime в pandas.Timestamp
        df = pd.read_csv(file_path)
        df['datetime'] = pd.to_datetime(df['datetime'])  # ОДНОКРАТНОЕ ПРЕОБРАЗОВАНИЕ
        
        # Теперь все datetime имеют методы .year, .month, .day
        # Проверяем, какие параметры нужно добавить
        params_to_add = []
        for cfg in params_config_new:
            if cfg['level'] == 0:
                col_name = f"{cfg['param']}_{cfg['agg']}"
            else:
                col_name = f"{cfg['param']}_{cfg['level']}hPa_{cfg['agg']}"
        ...
        # Добавляем параметры (все функции будут видеть pandas.Timestamp)
        df = add_parameters_batch_simple(df, params_to_add, data_type=data_type, km=km)
...
def add_parameters_batch_simple(df, params_config, data_type='LoRes', km=77):
    """Упрощенная версия, предполагающая что datetime уже в правильном формате"""
    results = {}
    ...
    for cfg in params_config:
        ...
        col_name = f"{param}_{level}hPa_{agg_type}" if level != 0 else f"{param}_{agg_type}"
        ...
        if data_type == 'ERA5':
            ...
            values = [get_mean_value_simple(df, i, level, param, agg_type, data_type, km) 
...
def get_mean_value_simple(CS, i, level, param, agg='median', data_type='LoRes', km=77):
    """Упрощенная версия, предполагающая что datetime уже в правильном формате"""
    hw = int(CS.loc[i, 'rad'])
    y = CS.loc[i, 'y']
    x = CS.loc[i, 'x']
    dt = CS.loc[i, 'datetime']  # уже должен быть pandas.Timestamp
    ...
    if data_type in ['HiRes', 'LoRes']:
        global ds0
        y_max = len(ds0.south_north) if hasattr(ds0, 'south_north') else ds0.dims['south_north']
        x_max = len(ds0.west_east) if hasattr(ds0, 'west_east') else ds0.dims['west_east']
        
        y_out = min(y_int + hw, y_max)
        x_out = min(x_int + hw, x_max)
        
        func = compute_aggregation_value_wrfout_simple
        
        return func(param, dt, y_in, y_out, x_in, x_out, 
                   our_level=level, data_type=data_type, km=km, agg=agg)
    return np.nan

def compute_aggregation_value_wrfout_simple(param, t, y_in, y_out, x_in, x_out, our_level, 
                                           data_type='LoRes', km=77, agg='min'):
    """Упрощенная версия, t уже pandas.Timestamp"""
    ds = load_wrf_file(t.year, t.month, t.day)
    hour_idx = t.hour // 3
    
    try:
        if param == 'wspd':
            u = wrf.getvar(ds, 'U10', timeidx=hour_idx)
            v = wrf.getvar(ds, 'V10', timeidx=hour_idx)
            ...
            var_data = wrf.getvar(ds, param, timeidx=hour_idx)
            data = var_data[our_level] if our_level < var_data.shape[0] else var_data[0]
        else:
            var_data = wrf.getvar(ds, param, timeidx=hour_idx)
            if our_level != 0 and len(var_data.shape) > 2:
                data = var_data[our_level] if our_level < var_data.shape[0] else var_data[0]
            else:
                data = var_data
        
        data_cropped = data[y_in:y_out, x_in:x_out]
        
        if param in ['cape_2d', 'cape_3d']:
            data_cropped = np.nan_to_num(data_cropped, nan=0.0)
        
        if agg == 'delta':
            return np.nanpercentile(data_cropped, 95) - np.nanpercentile(data_cropped, 5)
        elif agg == 'min':
            return np.nanmin(data_cropped)
        elif agg == 'max':
            return np.nanmax(data_cropped)
        elif agg == 'median':
            return np.nanmedian(data_cropped)
        else:
            return np.nanmedian(data_cropped)
...
def add_new_parameters_parallel(data_type, path_tracks_dir, params_config_new, km, output_suffix='_updated', n_workers=8):
    """Параллельная версия добавления параметров"""
    ...
    for year in years:
        for month in months:
            ...
            files = glob.glob(f'{original_path}/*.csv')
            
            # Создаем выходную директорию
            output_path = f'{path_tracks_dir}{output_suffix}/{year}-{month:02d}'
            os.makedirs(output_path, exist_ok=True)
            
            for file in files:
                all_files.append((file, params_config_new, data_type, km, output_path))
    
    print(f"Total files to process: {len(all_files)}")
    
    # Параллельная обработка
    with Pool(processes=n_workers) as pool:
        results = list(tqdm(pool.imap(process_single_track, all_files), 
                           total=len(all_files), 
...
def add_parameters_batch_optimized(df, params_config, data_type='LoRes', km=77):
    ...
    # Группируем параметры по уровню и типу для batch обработки
    params_by_level = {}
    for cfg in params_config:
        key = (cfg['level'], cfg['param'] == 'wspd')
        if key not in params_by_level:
            params_by_level[key] = []
        params_by_level[key].append(cfg)
    
    results = {}
    
    for (level, is_wspd), params_group in params_by_level.items():
        if data_type == 'ERA5':
            for cfg in params_group:
                param = cfg['param']
                agg_type = cfg['agg']
            ...
            # Для WRF/NAAD данных - batch обработка по времени
            # Группируем по дням для оптимизации загрузки файлов
            unique_dates = np.unique(datetimes.astype('datetime64[D]'))
            
            for date in unique_dates:
                mask = datetimes.astype('datetime64[D]') == date
                indices = np.where(mask)[0]
                
                # Загружаем файл один раз на день
                t_sample = datetimes[indices[0]]
                ds = load_wrf_file(t_sample.year, t_sample.month, t_sample.day)
                ...
                for cfg in params_group:
                    ...
                    if col_name not in results:
                        results[col_name] = [np.nan] * len(df)
                    
                    # Обрабатываем все точки за этот день
                    for i in indices:
                        value = get_mean_value_optimized(df, i, level, param, agg_type, ds, data_type, km)
                        results[col_name][i] = value
    
    # Добавляем все колонки сразу
    for col, values in results.items():
        df[col] = values
    
    return df

def get_mean_value_optimized(CS, i, level, param, agg, ds, data_type='LoRes', km=77):
    """Оптимизированная версия с переиспользованием Dataset"""
    hw = int(CS.loc[i, 'rad'])
    y = CS.loc[i, 'y']
    x = CS.loc[i, 'x']
    ...
    # Используем -1 для slice до конца
    y_out = None if y_int + hw >= ds.dims['south_north'] else y_int + hw
    x_out = None if x_int + hw >= ds.dims['west_east'] else x_int + hw
    
    if param == 'wspd':
        return compute_aggregation_wspd_optimized(param, CS['datetime'][i], y_in, y_out, x_in, x_out, 
                                                  level, data_type, km, agg, ds)
    else:
        return compute_aggregation_value_optimized(param, CS['datetime'][i], y_in, y_out, x_in, x_out, 
                                                   level, data_type, km, agg, ds)
...
def get_cached_region_data(file_path, param, level, y_in, y_out, x_in, x_out, time_idx):
    """Кэширует вырезанные области данных"""
    ds = get_cached_dataset(file_path)
    
    if level == 0:
        data = ds[param].isel(time=time_idx, 
                              south_north=slice(y_in, y_out), 
                              west_east=slice(x_in, x_out))
    else:
        data = ds[param].isel(time=time_idx, 
                              interp_level=level,
                              south_north=slice(y_in, y_out), 
...
def compute_aggregation_value_optimized(param, t, y_in, y_out, x_in, x_out, level, 
                                        data_type='LoRes', km=77, agg='min', ds=None):
    ...
    # Используем кэш для повторяющихся регионов
    cache_key = (id(ds), param, level, y_in, y_out, x_in, x_out, hour_idx)
    ...
    try:
        if param == 'wspd':
            ...
            v = wrf.getvar(ds, 'V10', timeidx=hour_idx)
            data = np.sqrt(u**2 + v**2)
            data_cropped = data[y_in:y_out, x_in:x_out] if y_out else data[y_in:, x_in:]
        else:
            if level == 0:
                data = wrf.getvar(ds, param, timeidx=hour_idx)
            else:
                data = wrf.getvar(ds, param, timeidx=hour_idx)[level]
        ...
        if agg == 'min':
            ...
            return np.nanmean(data_cropped)
        elif agg == 'std':
            return np.nanstd(data_cropped)
        else:
            return np.nanmedian(data_cropped)
