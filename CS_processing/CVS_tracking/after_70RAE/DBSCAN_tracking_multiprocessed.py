...
def process_tracking_config(config):
    ...
    for year, month in pbar:
        ...
        # path_data_dir = f"{path_dir_data}/TC_tracks/my_tracking_results/{pref_tracking}/{results_dir}/{data_type}_tracks_sigma_{sigma}/tracks_{circ}/{year}-{month:02d}/"
    ...
    for TC in CS_tracks_list:
        if np.sum(~np.isnan(TC['t'])) >= 3:
            ...
            cluster_idx = save_track_txt(cluster_idx, TC, path_data_dir)
