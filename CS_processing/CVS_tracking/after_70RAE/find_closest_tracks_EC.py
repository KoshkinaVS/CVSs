...
if data_type != 'SMP':
    ...
    tracking_types.append('tracking_local_only')
    max_distance_km = 5  # Максимальная допустимая дистанция (км)
...
def process_configuration(config):
    ...
    if data_type == 'HiRes':
        ...
        folder_NAAD = f'{path_data}/{data_type}/{data_type}_tracks_sigma_{sigma}/{pref_tracking}/{tracking_type}_{CVS_speed}_{pref_tracking}'
    else:
    ...
    for year in years:
        ...
        if data_type != 'SMP':
            ...
            # ДЕБАГ: Проверка создания TC_dict
            print(f"DEBUG: Created TC_dict with {len(TC_dict)} entries")




            TC_dict = process_all_tracks_fast(TC_dict, CS_tracks_list_NAAD, max_distance_km, max_time_diff, similar_steps)
        ...
        for key in tqdm(TC_dict.keys(), desc=f'Calculating distances {tracking_type} {CVS_speed} {year}'):
            ...
            for idx in range(len(TC_dict[key]['NAAD_tracks'])):
                ...
                if track_NAAD is not None:
                    ...
                    if data_type == 'SMP':
                        dist = get_tracks_dist(TC_dict[key]['NOAA_track'], track_NAAD, x_name='x', y_name='y', manual_data='EC')
                        dist_list.append(np.nanmean(dist)*6) ### умножила на 6 км
                    else:
        ...
        if data_type == 'SMP':
            folder_name = f'EC_tracks_merged/{data_type}_sigma_{sigma}/{tracking_type}_{CVS_speed}'
            save_TC_merged(TC_dict, f'{path_data}/{data_type}', folder_name, manual_data='EC')
            
            folder_name_pics = f'pics/EC_tracks_comparison/{data_type}_sigma_{sigma}/{tracking_type}_{CVS_speed}'
            for idx, key in tqdm(enumerate(TC_dict.keys()), desc=f'Plotting {tracking_type} {CVS_speed} {year}'):
                plot_closest_track_for_EC(TC_dict, key, 'NAAD_tracks', f'{path_data}/{data_type}', folder_name_pics, data_type)
        else:
