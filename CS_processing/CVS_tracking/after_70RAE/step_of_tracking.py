...
def get_init_params(data_type, json_file, print_info=False):
    ...
    if data_type == 'HiRes':
        ...
        dt_step = 24*3600 #seconds = 24 hours !!!!!!!
...
if data_type == 'GPN' or data_type == 'SMP' or data_type == 'ERA5':
    dt_step = 3600 #seconds = 1 hour !!!!!!!
...
TRACKING_METHODS: Dict[str, Callable] = {
    ...
    'tracking_local_only': None,         # Будет заполнено
    'tracking_local_2_phase_cone': None,         # Будет заполнено
}
...
def initialize_tracking_methods():
    ...
    from func_for_local_2_phase_cone import step_of_tracking as local_2_phase_cone_tracking
    ...
    TRACKING_METHODS['tracking_local_2_phase_cone'] = local_2_phase_cone_tracking
...
#                                "wspd": TC_wspd,
    ...
    folder = f"{path_data_dir}/{year}-{month:02d}/"
    os.makedirs(folder, exist_ok=True)
    
    df_TC_Danielle.to_csv(f'{folder}/{cluster_idx:06d}_track_{year}-{month:02d}-{day:02d}T{hour:02d}.csv')
    cluster_idx += 1
...
def save_track_txt(cluster_idx, TC, path_data_dir):
    """
    Сохраняет трек в формате TempestExtremes (только единый файл, без отдельных CSV)
    """
    start_date = TC['time'][0]
    if isinstance(start_date, np.datetime64):
        start_date = pd.Timestamp(start_date)
    ...
    # Создаем директорию для годовых файлов
    yearly_dir = f"{path_data_dir}/txt_yearly/"
    os.makedirs(yearly_dir, exist_ok=True)
    
    yearly_file = os.path.join(yearly_dir, f'tracks_{year}-{month:02d}.txt')
    
    # Более надежный подсчет реальных точек (без NaN)
    # Считаем только те точки, где t не NaN и x не NaN
    valid_points = []
    ...
    with open(yearly_file, 'a') as f:
        # Строка start - правильный формат: start <npoints> <year> <month> <day> <hour>
        f.write(f"start\t{npoints}\t{start_date.year}\t{start_date.month}\t{start_date.day}\t{start_date.hour}\n")
        
        # Точки трека (как в оригинале, с [:-1])
        for i in range(len(TC['t']) - 1):
            ...
            # Записываем в формате TempestExtremes
...
def get_all_coords(ds, t, circ, multi_levels=False):    
    if data_type == 'ERA5':
        ds_mini = ds.isel({time_unit: t, level_unit: 0})[['cluster', 'R2D']]
...
def get_stat_local_max(ds, t, circ, multi_levels=False):    
    if data_type == 'ERA5':
        ds_mini = ds.isel({time_unit: t, level_unit: 0})[['local_extr_crit', 'local_extr_cluster', 'local_extr_rad_eff']]
        X_arr = ds_mini.to_dataframe().dropna(how='any')
...
def get_stat_global_max(ds, t, circ, multi_levels=False):    
    if data_type == 'ERA5':
        ds_mini = ds.isel({time_unit: t, level_unit: 0})
        X_arr = ds_mini.to_dataframe().dropna(how='any')
...
def get_next_loc_cases(CS_tracks_list, ds, 
    ...
    hw_scale = 3 # для скорости можно брать шире область
    
    #учет скоростей перемещения
    ...
    #учет всех скоростей         
        ...
        if len(CS_tracks_list[i]['x']) == 1:
            ...
            # Координаты от скорости по треку (adv_speed)
            x_adv, y_adv = get_new_loc_CS_speed(ds, dt_step, x_init, y_init, x_prpr, y_prpr)
            
            # Координаты от фоновой скорости (bg_speed)
            x_bg, y_bg = get_new_loc_mean_speed(ds, data_type, dt_step, x_init, y_init, int(hw_scale*np.round(rad_init)), our_time, speed_level)
            
            # Полусумма (среднее арифметическое)
            x = (x_adv + x_bg) / 2.0
            y = (y_adv + y_bg) / 2.0
    # неучет скоростей                    
    else:
...
def track_init(ds, clstr_len, data_type, stat_Q, CS_tracks_list, our_time, time_name='XTIME', multi_levels=False):
    for i in range(len(stat_Q)):
        ...
        if data_type == 'ERA5':
            lon_coords.append(float(stat_Q.lon.values[i]))
            lat_coords.append(float(stat_Q.lat.values[i]))
        elif data_type == 'SMP':
