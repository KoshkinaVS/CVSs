...
# from wrf import getvar, interplevel,ll_to_xy # ошибка с np.float
...
def get_rain_with_previous(ds_current, time_idx, current_time, wrf_files):
    ...
    if time_idx > 0:
        ...
        if prev_file:
            ...
            print(f"    Нет предыдущего файла для {current_time}")
...
def get_values_in_radius(data, center_lat_idx, center_lon_idx, radius_pixels, method=''):
    try:
        ...
        if data_values.ndim == 2:
            ...
            print(f"Неподдерживаемая размерность: {data_values.ndim}")
            return np.nan
...
def get_single_level_values(data_2d, center_lat_idx, center_lon_idx, radius_pixels, method=''):
    ...
    try:
        ...
        if method == 'mean':
            ...
            return np.std(valid_data, ddof=1)
        elif method == 'point':
...
def process_single_track_wrapper(args):
    ...
    try:
        ...
        print(f" Ошибка обработки трека {os.path.basename(track_file)}: {e}")
...
def calculate_track_statistics(df, track_filename):
    ...
    # Добавляем КИЛОМЕТРАЖ ВИХРЯ (в км)
    total_length = sum(
            distance(coords[i-1], coords[i]).kilometers 
            for i in range(1, len(coords))
        )
    stats['track_length_km'] = total_length
...
def process_single_track(track_file, wrf_files, output_dir, data_type='EC'):
    ...
    try:
        ...
        all_params = [
            #stoll_params
            'SLP_center', 'SLP_median','SLP_diff(cent-med)','SLP_95','SLP_diff(cent-95)',

            'U10_mean_cyclone', 'U500_mean', 'U500_poleward_cyclone', 'PV_850_mean_cyclone',
            'T500_mean', 'T700_mean', 'SST_mean','theta_e_700_mean', 'theta_e_850_mean',
            ...
            'theta_e_SST_minus_theta_e_850_mean','MCAO1_500_mean', 'MCAO1_700_mean', 'MCAO2_mean', 'grad_theta_e_850_mean','PBL_med', 'rel_vor_850_med',
            ...
            #custom_params
            'HFX_rad', 'LH_rad', 'mucape_95', 'cape_sur', 'helicity_rad', 'pw_rad',
            'w_10m_mean', 'N_mean','propagation_speed','differential_wind_vector',
            'vertical_shear_strength','alpha_d','alpha_p','vertical_shear_angle', 'vertical_shear_vector_u','vertical_shear_vector_v','RAIN_HOURLY',
            'wind_shear_10m_500',
            'T850','T2', 'T2_disp' ,'SST_disp','T850_disp',
            #bergeron_indexes
            'bergeron_2h', 'bergeron_4h', 'bergeron_6h', 'bergeron_12h','bergeron_24h',
            #others form list
            'george_index','LI_rad','z500_rad','U200_95','Q850_95','LCL_95','LFC_95','LFC-LCL','mcin_95', 'cin_sur', 'DBZ_sfc_500_mean',
...
def process_single_hour(i, row, wrf_files, storm_data):
    ...
    try:
        ...
        with Dataset(selected_file) as ds:
            ...
            # center_lat_idx, center_lon_idx = find_grid_indices_fast(center_lat, center_lon, lats, lons)
            ...
            """
            # xy = ll_to_xy(ds, center_lat, center_lon, timeidx=time_idx)
            # center_lon_idx, center_lat_idx = xy[0], xy[1]
            ...
            '''_________РАСЧЕТ ШТОЛЛОВСКИХ ПАРАМЕТРОВ_________'''
            '''_______________________________________________'''
            try:
                # 1. SLP (hpa)
                ...
                # uvmet10 - (2, lat, lon) - U и V компоненты
                wind_speed = np.sqrt(wind_10m[0]**2 + wind_10m[1]**2)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['U10_mean_cyclone'] = get_values_in_radius(wind_speed, center_lat_idx, center_lon_idx, radius_pixels, 'median')
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['U500_poleward_cyclone'] = get_poleward_sector(wind_speed_500, center_lat_idx, center_lon_idx, radius_pixels, lats, 'median')
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

                # 5. Вихренность на 850 гПа (PVU)
                ...
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['PV_850_mean_cyclone'] = get_values_in_radius(pot_vorticity_850, center_lat_idx, center_lon_idx, radius_pixels, 'median')
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                # 6. Температуры (K)
                ...
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['grad_theta_e_850_mean'] = get_poleward_sector(
                    grad_theta_e_magnitude, center_lat_idx, center_lon_idx, radius_pixels, lats, 'median')
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                '''_________РАСЧЕТ КАСТОМНЫХ ПАРАМЕТРОВ__________'''
                '''______________________________________________'''
                # 0. DELTA SLP
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                try:
                    ...
                    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                    results['helicity_rad'] = get_values_in_radius(srhel, center_lat_idx, center_lon_idx, radius_pixels, 95)
                    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                except:
                    results['helicity_rad'] = np.nan
                
                # 5. Precipitable Water
                pw = getvar(ds, "pw", timeidx=time_idx)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['pw_rad'] = get_values_in_radius(pw, center_lat_idx, center_lon_idx, radius_pixels, 95)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                w_surface = w[0,:,:]
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['w_10m_mean'] = get_values_in_radius(w_surface, center_lat_idx, center_lon_idx, radius_pixels, 95)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                try:
                    ...
                    # 500 гПа ветер (предполагаю, что выше уже есть u500interp, v500interp)
                    u500np = u500interp.values if hasattr(u500interp, "values") else u500interp
                    v500np = v500interp.values if hasattr(v500interp, "values") else v500interp
                    speed500 = np.sqrt(u500np**2 + v500np**2)
                ...
                u_925 = interplevel(u, pres_3d, 925., meta=True)
                v_925 = interplevel(v, pres_3d, 925., meta=True)
                ...
                if not np.isnan(mean_dV_magnitude):
                    ...
                    delta_z_mean = get_values_in_radius(delta_z, center_lat_idx, center_lon_idx, radius_pixels, 'median')
                ...
                try:
                    ...
                    '''______________________________________________'''
                    '''_________РАСЧЕТ ДИСПЕРСИИ ПАРАМЕТРОВ (пока температуры)__________'''
                    '''______________________________________________'''
             
                # 12. Загружаем T2
                T2 = getvar(ds, "T2", timeidx=time_idx)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['T2'] =get_values_in_radius(T2, center_lat_idx, center_lon_idx, radius_pixels, 'median')
                results['T2_disp'] = get_values_in_radius(T2, center_lat_idx, center_lon_idx, radius_pixels, 'std')
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                
                # 13. T850 + его дисперсия
                t_850 = interplevel(temperature, pressure, 850., meta=True)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['T850'] = get_values_in_radius(t_850, center_lat_idx, center_lon_idx, radius_pixels, 'median')
                results['T850_disp'] = get_values_in_radius(t_850, center_lat_idx, center_lon_idx, radius_pixels, 'std')
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                
                # 14. SST дисперсия
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['SST_disp'] = get_values_in_radius(sst_data, center_lat_idx, center_lon_idx, radius_pixels, 'std')
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

                '''______________________________________________'''
                '''_________РАСЧЕТ ОСТАЛЬНЫХ ПАРАМЕТРОВ С СПИСКА__________'''
                '''______________________________________________'''
                ...
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['george_index'] = get_values_in_radius(k_index, center_lat_idx, center_lon_idx, radius_pixels, 95)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

                # 16. '''Lifted Index (LI)'''
                'LI = T500(окружающий воздух) --- T500(поднятая частица)     -95 процентиль-' 
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['LI_rad']=calculate_li(center_lat_idx,center_lon_idx,t_500,T2,pressure,radius_pixels,dewpoint_sst)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                z_500 = interplevel(z,     # поле для интерполяции
                     ...
                     meta=True)  # сохранить метаданные
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['z500_rad']=get_values_in_radius(z_500, center_lat_idx, center_lon_idx, radius_pixels, 95)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['U850_95'] = get_values_in_radius(wind_speed_850, center_lat_idx, center_lon_idx, radius_pixels,95 )
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                results['U200_95'] = get_values_in_radius(wind_speed_200, center_lat_idx, center_lon_idx, radius_pixels, 95)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                if rain_hourly is not None:
                    results['RAIN_HOURLY'] = get_values_in_radius(
                        rain_hourly, center_lat_idx, center_lon_idx, radius_pixels, 95
                    )
                else:
                    results['RAIN_HOURLY'] = np.nan
                ...
                results['Q850_95'] = get_values_in_radius(q_850, center_lat_idx, center_lon_idx, radius_pixels, 95)
                ...
                # print(dbz_vert_avg)
                results['DBZ_sfc_500_mean'] = get_values_in_radius(dbz_vert_avg, center_lat_idx, center_lon_idx, radius_pixels, 95)
                ...
                # print(rh_vert_avg)
                results['rh_95'] = get_values_in_radius(rh_vert_avg, center_lat_idx, center_lon_idx, radius_pixels, 95)

                #24. Updraft helicity
                try:
                    up_hel=getvar(ds,'updraft_helicity', timeidx=time_idx)
                    results['updraft_helicity'] = get_values_in_radius(up_hel, center_lat_idx, center_lon_idx, radius_pixels, 95)
                except: 
                ...
                results['INTEGR_VAPOR_TRANSP'] = get_values_in_radius(ivt, center_lat_idx, center_lon_idx, radius_pixels, 95)
                results['U_VAPOR_TRANS'] = get_values_in_radius(ivt_U, center_lat_idx, center_lon_idx, radius_pixels, 95)
                results['V_VAPOR_TRANS'] = get_values_in_radius(ivt_V, center_lat_idx, center_lon_idx, radius_pixels, 95)
