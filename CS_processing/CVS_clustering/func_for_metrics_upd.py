...
def get_ordered_metrics(metrics_df):
    """Только выбранные метрики качества кластеризации."""
...
def plot_metrics_subplot(fig, metrics_df, seasons_to_plot, colors, common_metrics):
    """Подграфики для 4 выбранных метрик, все в один ряд."""
    ...
    all_metrics = [m for m in all_metrics if m in metrics_df.columns]
    ...
    for i, metric in enumerate(all_metrics):
        ...
        for season_name, color in zip(seasons_to_plot, colors):
            data_season = metrics_df[
                metrics_df['season'] == season_name
            ]

            if metric not in data_season.columns:
                continue

            # Silhouette и stability — обычные линии
            if metric not in ['classifiability', 'reproducibility']:

                plt.plot(
                    data_season['n_clusters'],
                    data_season[metric],
                    marker='o',
            ...
            # цвет точек зависит от p-value
            else:

                p_colors = get_pvalue_colors(
                    data_season,
                    metric
                )
        ...
        plt.xticks(
            sorted(metrics_df['n_clusters'].unique())
        )
...
def compute_metrics(X, labels,
    ...
    """
    Вычисляет выбранные метрики кластеризации:
    ...
    gmm_metrics и clustering_type оставлены для совместимости
    с основным скриптом.

    compute_classifiability_flag=False:
        classifiability считается,
        p-value относительно шума не считается.

    compute_reproducibility_flag=False:
        reproducibility считается,
        p-value относительно шума не считается.
    """
    ...
    if compute_classifiability_flag:
        ...
        classif_val = compute_classifiability_index(
            X_clean,
            n_clusters,
            M=noise_M_class,
            random_state=random_state
    ...
    if compute_reproducibility_flag:
        ...
        repr_val = compute_reproducibility_index(
            X_clean,
            n_clusters,
            R=noise_R_repr,
            M=noise_M_repr,
    ...
    metrics = {
        ...
        'classifiability': classif_val,
        'classifiability_p': classif_p,

        'reproducibility': repr_val,
        'reproducibility_p': repr_p,
    }
