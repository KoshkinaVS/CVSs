...
path_dir_raw = f"/storage/thalassa/DATA/ERA5/PL/grib"
...
for year in tqdm(years, desc="Years"):
    ...
    pattern = f'{path_dir_raw}/{year}/era5_pl_{year}-*.grib'
    ...
    for ncfile in all_input_files:
        ...
        u = ds[params[0]] #.values[0,0]
        v = ds[params[1]] #.values[0,0]
...
#                       y_name: len(ds_day[y_name]), x_name: len(ds_day[x_name])}, r2d.astype(np.float32))
        ...
        print(f"{year}-{month:02d}-{day:02d} обработан за {elapsed:.2f} минут")