...
#### 2026-08-18 добавила аналогично коду для NAAD - радиусы в терминах ячейки сетки (надо будет домножать на косинус широты)
...
def get_DBSCAN_ds(ds_HiRes, config, our_level=0, data_type='LoRes'):
    ...
    for idx, t in enumerate((times)):
        ...
        #### 2026-08-18 добавила аналогично коду для NAAD - радиусы в терминах ячейки сетки (надо будет домножать на косинус широты)
        local_max = update_rad(local_max, coords)
    ...
    if data_type != 'ERA5':
        del result.coords['south_north']
        del result.coords['west_east']
