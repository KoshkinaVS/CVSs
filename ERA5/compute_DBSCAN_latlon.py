...
# Configuration
with open('config_ERA5_heights.json', 'r') as file:
...
rortex_path = f'{path_dir_data}/{data_type}/{name_crit}_{data_type}_heights/'
output_folder = f'{path_dir_data}/{data_type}/DBSCAN_{config["eps"]:02d}-{config["min_samples"]:02d}-{config["size_filter"]:02d}_heights/'

if smooth:
    rortex_path = f'{path_dir_data}/{data_type}/{name_crit}_{data_type}_heights_sigma_{sigma}/'
    output_folder = f'{path_dir_data}/{data_type}/DBSCAN_{config["eps"]:02d}-{config["min_samples"]:02d}-{config["size_filter"]:02d}_heights_sigma_{sigma}/'
...
def main():
    ...
    for year in tqdm(years, desc="Scanning years"):
        for month in range(1, 13):
            pattern = f'{rortex_path}/{name_crit}_{data_type}_{year}-{month:02d}-*.nc'
            if smooth:
                pattern = f'{rortex_path}/sigma_{sigma}_{name_crit}_{data_type}_{year}-{month:02d}-*.nc'
