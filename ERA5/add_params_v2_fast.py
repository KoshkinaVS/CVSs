...
3. ПАРАЛЛЕЛИЗМ ПО СУТКАМ.
   Точки группируются по календарной дате (это естественная граница
   GRIB-файла — один файл = одни сутки) и раздаются по процессам через
   `ProcessPoolExecutor`. Каждый воркер обрабатывает свой набор суток
   целиком, поэтому кэш открытых Dataset внутри воркера работает эффективно
   (сутки не "перескакивают" между процессами).
...
class CycloneProcessorERA5:
    def __init__(
        self,
        data_paths: dict,
        radius_multiplier: float = 4.0*1.5, # to degrees (4) + huge area (1.5)
...
def process_all_tracks(
    ...
    # ------------------------------------------------------------------
    # 2. Группировка точек по календарным суткам (= граница GRIB-файла).
    # ------------------------------------------------------------------
    ...
    logger.info(
        "Всего точек: %d; уникальных суток: %d; воркеров: %d",
        len(points),
        len(day_keys),
        n_workers,
    )

    day_batches = [day_buckets[k] for k in day_keys]

    # ------------------------------------------------------------------
    # 3. Обработка (параллельно по суткам или последовательно при n_workers<=1).
    # ------------------------------------------------------------------
    ...
    if n_workers <= 1:
        ...
        for batch in tqdm(day_batches, desc="Дни (последовательно)"):
            all_results.extend(processor.process_day_points(batch))
    else:
        ...
        with ctx.Pool(
            ...
            for batch_result in tqdm(
                pool.imap_unordered(_worker_process_day, day_batches),
                total=len(day_batches),
                desc="Дни (параллельно)",
            ):
                all_results.extend(batch_result)

    # ------------------------------------------------------------------
    # 4. Записываем результаты обратно в DataFrame-ы треков.
    # ------------------------------------------------------------------
    for track_id, row_idx, results in all_results:
...
if __name__ == "__main__":
    ...
    OUTPUT_PATH = Path(f"{TRACKS_PATH}_params_2026-08-24")
