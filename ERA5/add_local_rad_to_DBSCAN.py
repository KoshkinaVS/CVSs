...
os.makedirs(f'{output_folder}_upd', exist_ok=True)
...
for year in tqdm(years, desc="Years"):
    for month in tqdm(np.arange(1, 13, 1), desc="Months", leave=False):
        ...
        pattern = f'{output_folder}/sigma_{sigma}_DBSCAN_{data_type}_{year}-{month:02d}-*.nc'
        ...
        for ncfile in all_input_files:
            cluster_ds = xr.open_dataset(ncfile)
            day = cluster_ds[time_name].dt.day.values[0]
            
            output_file = f'{output_folder}_upd/sigma_{sigma}_DBSCAN_{data_type}_{year}-{month:02d}-{day:02d}.nc'
