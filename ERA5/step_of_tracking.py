...
def get_all_coords(ds, t, circ):    
    ...
    # Получаем индексы (x, y) и координаты (lat, lon)
    ...
    coords = pd.DataFrame({
        ...
        'lat': index_df['latitude'].values,  # значения latitude
        'lon': index_df['longitude'].values,  # значения longitude
...
def dist_ERA5(p1_lon, p1_lat, p2_lon, p2_lat):
    return geodesic((p1_lat, p1_lon), (p2_lat, p2_lon)).kilometers
...
def get_new_loc_mean_speed(ds, data_type, dt, x, y, hw, t, speed_level):
    ...
    # учет широт-долгот
    dx = dist_m * np.cos(np.radians(ds[y_unit].values[y]))
...
def track_init(ds, clstr_len, data_type, stat_Q, CS_tracks_list, our_time, time_name='XTIME'):
    ...
    for i in range(len(stat_Q)):
        ...
        if data_type == 'ERA5':
            lat = stat_Q.lat.values[i]
            lon = stat_Q.lon.values[i]
...
def append_nan_to_track(track):
    """Helper function to append NaN values to a track"""
